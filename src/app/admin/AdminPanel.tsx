    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
      ...
      <form ref={formRef} action={formAction} className="mt-4 flex flex-col gap-3">
        <input
          ...
          autoComplete="off"
          className="rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-base text-white outline-none focus:border-amber-500"
        />
        <input
          name="email"
          type="email"
        ...
        {state?.ok && (
          <p className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300">
            User created. Text them their login below.
          </p>
        )}
        <button
          type="submit"
          disabled={pending}
          className="rounded-xl bg-amber-500 px-4 py-3 text-base font-semibold text-slate-950 transition active:scale-[0.99] disabled:opacity-60"
        >
...
function buildInstructions(email: string): string {
  const origin =
    typeof window !== "undefined" ? window.location.origin : "the app link";
  return [
    ...
    "",
    "Add it to your home screen so it works like an app:",
    ...
    <li className="rounded-xl border border-slate-800 bg-slate-900/50 p-3">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          ...
          className="rounded-lg bg-amber-500 px-3 py-1.5 text-xs font-semibold text-slate-950 transition active:scale-95 disabled:opacity-40"
        >
          Text instructions
        </button>
        <button
